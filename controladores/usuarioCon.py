...
class RegistrarUsuario:
    ...
    def subirUsuario(self, Id, usuario, contrasena, nombre, apellido, puesto):
        self.conn = conecciones()
        with self.conn.cursor() as cursor:
            sql = """UPDATE usuario SET usuario = %s, contrasena = %s, nombre = %s,
            apellido = %s, puesto = %s WHERE idUsuario = %s """
            cursor.execute(sql, (usuario, contrasena, nombre, apellido, puesto, Id))
            self.conn.commit()
